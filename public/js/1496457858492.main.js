...
function masonryReload(data) {
	"use strict";
	$('.w-modal-folder-inner').html(data)
		.masonry( {itemSelector: '.w-img-select-thumb'})
		.imagesLoaded(function(){
				$('.w-modal-folder-inner').masonry( {itemSelector: '.w-img-select-thumb'});
				$('.w-modal-folder-loader').toggle();
			})
		.masonry('reloadItems');
}

function masonryFill(selector) {
	"use strict";
    $(selector).imagesLoaded(function(){
        $(selector).masonry({ itemSelector: '.w-img-select-thumb' });
        $('.w-modal-folder-loader').hide();
    }).masonry('reloadItems');
}
...
function initMap() {
    ...
    $(document).find('.googlemap-api').each(function(){
	    ...
	    var mapzoom = parseInt($(this).data('map-zoom'), 10)
  		var mapcoordslat = parseInt($(this).data('map-coords-lat'), 10)
  		var mapcoordslng = parseInt($(this).data('map-coords-lng'), 10)
